...
rm(list = ls())
setwd("C:/Users/Pedro/Dropbox/Special_Topics_in_Time_Series_Econometrics_2022/applied_economics_forecasting/appliedeconomicforecasting_R/cap1")
...
plot1 <- ggplot(sim.data$est, aes(x = 102:301, y = y)) + 
  geom_line() + 
  theme_bw() + 
  xlab('') + 
  ylab('') + 
...
plot2 <- ggplot(sim.data$est, aes(x = 102:301, y = x)) + 
  geom_line() + 
  theme_bw() + 
  xlab('') + 
  ylab('') + 
...
plot3 <- ggplot(sim.data$est, aes(x = x, y = y)) + 
  geom_point() + 
  theme_bw() + 
  xlab('X') + 
  ylab('Y')
