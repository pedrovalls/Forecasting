...
#'
#' title: "Cap 4 Forecast Evaluation and Combination"
#' author: 
#'  - "Pedro Valls"
...
rm(list = ls())
#setwd("C:/Users/Pedro/Dropbox/Special_Topics_in_Time Series_Econometrics_2022/applied_economics_forecasting/appliedeconomicforecasting_R/cap4")
...
sim.data <- read.csv('simulated_datac4.csv')
...
print(xtable(mgn.table), include.colnames = T)

#'
#' ##** p-values for Morgan-Granger-Newbold test**
#' 
...
#'
#' ##** p-value for recursive Diebold-Mariano test**
#' 

DM.table

#'
#' ## **p-value for recursive Morgan-Granger-Newbold test**
#' 
MGN.table
...
#'
#'## Morgan-Granger-Newbold test Simple and Recursive
#'
gdp.MGN
...
#' 
#' ## Results of Diebold-Mariano test Simple and Recursive
#' 
DM.table
#'
#'## Morgan-Granger-Newbold test
...
#' 
#' ## Results of Morgan-Granger-Newbold test Simple and Recursive
#' 
GDP.MGN
...
oas.data$OASnew = lead(oas.data$OAS,n=1)
oas.data[, Date := as.Date(Date, '%m/%d/%Y')]
oas.data <- oas.data[1:215]
View(oas.data)
model <- c('OASnew ~ OAS + VIX', 'OASnew ~ OAS + SENT','OASnew ~ OAS + PMI', 'OASnew ~ OAS + sp500')
oas.fit <- list()
